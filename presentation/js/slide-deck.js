...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var twitter = p.twitter ? '<b>twitter:</b> ' +
          '<a href="http://twitter.com/' + p.twitter + '">' +
          p.twitter + '</a>' : '';

      var email = p.email ? '<b>email:</b> ' + p.email + ' ' : '';

      var www = p.www ? '<b>web:</b> <a href="' + p.www +
                        '">' + p.www.replace(/https?:\/\//, '') + '</a>' : '';
	  var slideUrl = p.slideUrl ? '<b>slides:</b> <a href="' + p.slideUrl +
                 '">' + p.slideUrl.replace(/https?:\/\//, '') + '</a>' : '';

      var github = p.github ? '<b>github:</b> <a href="' + p.github +
          '">' + p.github.replace(/https?:\/\//, '') + '</a>' : '';

	  var jsfiddle = p.jsfiddle ? '<b>jsfiddle:</b> <a href="' + p.jsfiddle +
	          '">' + p.jsfiddle.replace(/https?:\/\//, '') + '</a>' : '';

      var html2 = [www, email, slideUrl, jsfiddle, github, twitter].join('<br>');
